...
`N` are `na::RealField` or `na::ComplexField` from the
...
The following examples won't compile, as the `OVector` type is a placeholder
for either static or dynamic type, but they must all match:
...
```
For static vectors, the dimensions are check at compile-time:
...
This previous code will fail to compile:
...
However, if your are passing dynamic vectors, no check will be perform
and the code will panic at runtime as your trying to add two vectors of different sizes:
...
fn main() {
    ...
    assert_eq!(dynamic_equations.difference(), nalgebra::vector![-1.0, -1.0]);
}
```
