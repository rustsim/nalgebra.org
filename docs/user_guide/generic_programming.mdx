...
with the first argument matching the second argument.
If you add the following line in the previous main, it will fail to compile:
...
one has to indicate the type for which it is implemented, either:
- `nalgebra::Dynamic`
- `nalgebra::Const`
...
Matrices works the same as vectors.
You have the types `DMatrix`, `SMatrix` and `OMatrix`.
